...
function checkAccountantAuth() {
    ...
    console.log('=== Accountant Auth Debug ===');
    ...
    if (user.role !== 'accountant' && user.role !== 'admin') {
        console.log('Wrong role - Access denied. Role:', user.role);
        showToast('Access denied. Accountant only. Your role: ' + user.role, 'error');
        setTimeout(() => {
            window.location.href = '../../pages/Authentication/login.html';
        }, 2000);
        return false;
    ...
    console.log('Auth check PASSED for role:', user.role);
    return true;
}
